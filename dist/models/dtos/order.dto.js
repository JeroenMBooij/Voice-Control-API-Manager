...
const endpoint_1 = require("../contract-models/endpoint");
let orderSchema = new mongoose.Schema({
    _id: mongoose.Schema.Types.ObjectId,
    command: { type: String, required: true },
    language: { type: String, required: true },
    action: { type: endpoint_1.Endpoint, required: true },
    authentication: { type: endpoint_1.Endpoint }
});
orderSchema.path('command').validate(function (value) {
    //make sure command does not contain too many spaces
    let invalidCommands = value.match(/[ ]{2}/g);
    if (invalidCommands) {
